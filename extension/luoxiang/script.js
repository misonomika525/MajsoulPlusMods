  ...
  try {
    ...
    Object.entries(cfg.voice.sound.groups_).forEach(([soundID, soundGroup]) => {
      if (soundID == 15) {
        const changeMap = {
          ...
          "大厅交互语音2": {
            "words": '前几天二阶堂美树咨询我一个法律问题，我吓了一跳，就跟这有关。她说“轻库娘，我问您一个法律问题——我想开个妓院行不行？”我吓一跳，我说你什么人嘛……'
          },
          "大厅交互语音3": {
    ...
    cfg.item_definition.character.rows_.forEach(chr => {
      ...
      switch (chr.id) {
        case 200015:
          helper('desc_cv', '罗翔');
          helper('desc', '某校濒临解散的文学部部长，不知名小说家，著名刑法学家，魂天法考刑法主讲。虽然写小说完全是余裕余裕，但一天的时间大多都是花在读书上，而把文学部的事情全然丢给部员。她的小说多以写实风格为主，从正面反映社会中的种种丑恶现象。不写小说的时候也会就时事在网上发表自己的观点，也由于观点经常一针见血而被某些网友攻击谩骂。代表作：《主客观相统一》、《王座之间的出与入》、《一姬的365种死法》和《拔北正义》等。');
          helper('desc_hobby', '读书（主要是小说）、刑法学研究');
          break;
      }
    });
  } catch (error) {
